...
def parked(monkeypatch):
    ...
    async def record(event, attempts, last_error):
...
def test_first_attempt_success(parked):
    calls = responses(200)
    result = run(meta_capi.deliver_or_park(EVENT))
...
def test_test_event_code_is_attached_when_set(monkeypatch):
...
def test_backoff_delays_are_1_3_9_seconds(monkeypatch, parked):
    ...
    result = run(meta_capi.deliver(EVENT))
...
def test_resend_mode_is_a_single_attempt():
    ...
    result = run(meta_capi.deliver(EVENT, retry=False))
...
def test_dispatch_is_a_noop_when_disabled(monkeypatch):
